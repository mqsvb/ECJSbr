...
function main() {
	...
	var selectEsp = InitOperation("CH-WIRE V2", "", 0);
	var printAll

	AddUserModToOperation(selectEsp, "_realEsp", "Espessura MDF", "", 0, "180");
	AddUserModToOperation(selectEsp, "_radioTool", "Ferramenta^08-1^DM45G^GRAU-45", "^Select Tool", 0, "");
	//AddUserModToOperation(selectEsp, "_int_MyModifier", "my integer", "" , 0, "");
	...
	if (nPickTool == 1) { printTool = "08-1" };
	if (nPickTool == 2) { printTool = "DM45G" };
	if (nPickTool == 3) { printTool = "GRAU-45" };
	...
	SetModifier(cmd0, 35, -nselectEsp);
	...
	SetModifier(cmd0, 28, nselectEsp+12);
	// Setting modifier 'Level'
	SetModifier(cmd0, 161, nselectEsp);
	// Setting modifier 'Depth'
	SetModifier(cmd0, 9, -nselectEsp);
	// Setting modifier 'Cut Increment'
	SetModifier(cmd0, 10, nselectEsp/2);
